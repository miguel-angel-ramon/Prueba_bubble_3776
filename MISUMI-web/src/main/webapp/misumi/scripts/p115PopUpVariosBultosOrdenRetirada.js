...
$(document).ready(function(){
	initializeScreenP115PopupVariosBultos();
	event_btn_guardadoVariosBultos();
	regexForInput();
	loadP115(locale);
...
/*********************************************************************************************************************************************/
/*********************************************************************************************************************************************/
function loadP115(locale){
	this.i18nJSON = './misumi/resources/p115PopUpVariosBultos/p115PopUpVariosBultos_' + locale + '.json';
...
function initializeScreenP115PopupVariosBultos(){
	$( "#p115_popUpVariosBultos").dialog({
		autoOpen: false,
		height: 'auto',
		width: 'auto',
		modal: true,
		resizable: false,
		open: function() {
			document.getElementById("p115_lbl_Error").style.display = "none";
			document.getElementById("p115_lbl_Error2").style.display = "none";
			document.getElementById("p115_lbl_Error3").style.display = "none";
		},
		close: function( event, ui ) {	

			//Limpiamos el popup p115
			limpiarPopUpP115();
		},
...
function limpiarPopUpP115(){	
	for(var i = 0; i<5; i++){
		var campoCantidad="p115_fld_cantidad"+i;
		var campoBulto="p115_fld_bulto"+i;
		document.getElementById(campoCantidad).value="";
		document.getElementById(campoBulto).value="";
	}
}

//Abrimos el popup p115 
function openPopUpVariosBultos(fila,tabla,nombrePopUp){			
	//Guardamos en una variable oculta, el campo en el que estamos para poder posicionarnos en el posteriormente al guardar.
	$("#p115_fld_StockDevuelto_Selecc").val(fila + "_stockDevuelto");
	$("#p115_fld_StockDevueltoOrig_Selecc").val(fila + "_stockDevuelto_orig");
	$("#p115_fld_StockDevueltoTmp_Selecc").val(fila + "_stockDevuelto_tmp");
	$("#p115_fld_Bulto_Selecc").val(fila + "_bulto");
	
	$("#p115_popUpVariosBultos").dialog("open");			
	...
	$("#p115_lbl_refDescripcion").text(refDescripcion);	

	var devolucionId = "#"+fila+"_devolucion";
	var devolucionPantalla = $(devolucionId).val();
	
//	ref=972013;
	cargarDatosPantalla(fila,ref,devolucionPantalla);
	
	//Ponemos el cursor en el primer campo de stock de la pantalla de varios bultos.
	$("#p115_fld_cantidad0").focus();
}

//Cuando pulsamos guardar, se validan los campos de cantidad y bulto 
function event_btn_guardadoVariosBultos(){
	$("#p115_btn_guardado").click(function(e){
		if(validarCantidadesyBultosP115()){
			if(hayBultosIgualesP115()){
				...
				document.getElementById("p115_lbl_Error2").style.display = "block";
				document.getElementById("p115_lbl_Error").style.display = "none";
				document.getElementById("p115_lbl_Error3").style.display = "none";
			}else{
				...
				listBultoCantidadPantalla=rellenarListaDatosPantalla();
				
				var objJson = prepareObjectJSON();
				$.ajax({
					url : './devoluciones/popup/ordenDeRetirada/guardarBultoCantidad.do',
					type: 'POST',
					...
					success : function(data) {
						if(data != null ){
							if(data.codError ==1){
								...
								document.getElementById("p115_lbl_Error3").style.display = "block";
								document.getElementById("p115_lbl_Error2").style.display = "none";
								document.getElementById("p115_lbl_Error").style.display = "none";
							}else{
								...
								let valorBultoSelec=$("#p115_fld_Bulto_Selecc").val();
								...
								// Limpio los estados del grid y asigno el estado guardar.
								...
								if ($("#p115_fld_StockDevuelto_Selecc").val() != '')	{
									if(sumatorioCantidad!=null){
										$("#"+$("#p115_fld_StockDevuelto_Selecc").val()).val($.formatNumber(sumatorioCantidad,{format:"0.###",locale:"es"}));	
										$("#"+$("#p115_fld_StockDevueltoOrig_Selecc").val()).val($.formatNumber(sumatorioCantidad,{format:"0.###",locale:"es"}));
										$("#"+$("#p115_fld_StockDevueltoTmp_Selecc").val()).val($.formatNumber(sumatorioCantidad,{format:"0.###",locale:"es"}));
									}
									
								}
								
								if ($("#p115_fld_Bulto_Selecc").val() != '')	{
									$("#"+$("#p115_fld_Bulto_Selecc").val()).val(bultoStr.substring(0,bultoStr.length-1));
								}
								...
								var devolucionEditada = new Devolucion(null,null,null,null,null,null
																	  ,$("#p115_fld_devolucion").val(),null,null
								...
								recargaCmbProvSelecP84(objJsonProv,provSeleccionado);
								// ***********************************************************************************/
			...
			document.getElementById("p115_lbl_Error").style.display = "block";
			document.getElementById("p115_lbl_Error2").style.display = "none";
			document.getElementById("p115_lbl_Error3").style.display = "none";
		}
	});
...
function prepareObjectJSON(){
	var devolucionPantalla = $("#p115_fld_devolucion").val();
	var codArtPantalla = $("#p115_fld_codArt").val();
	var datosPantalla = {
...
function rellenarListaDatosPantalla(){
	var listaBultoCantidadPantalla = new Array();
	...
	for(var i = 0; i<5; i++){
		...
		let campoCantidad="p115_fld_cantidad"+i;
		let campoBulto="p115_fld_bulto"+i;
		...
		registroListadoModificado.estadoCerrado=$("#p115_estadoCerrado"+i).val();
...
function cargarDatosPantalla(fila,ref,devolucionPantalla){
	$.ajax({
		type : 'GET',	
		url : './devoluciones/popup/ordenDeRetirada/cargarDatosPantallaVariosBultos.do'+ createParamsUrl(ref,devolucionPantalla),
		success : function(data) {	
			if(data != null){
				$("#p115_fld_devolucion").val(data.devolucion);
				$("#p115_fld_codArt").val(data.codArticulo);
				$("#p115_fld_flgPesoVariable").val(data.flgPesoVariable);
				$("#p115_listaBultos").val(data.listaBultos);

				if(data.flgPesoVariable=='S'){
					document.getElementById("p115_lbl_avisoPesoVariable").style.display = "block";
				}else{
					document.getElementById("p115_lbl_avisoPesoVariable").style.display = "none";
				}
...
//	            let cellEstadoCerradoValue = $("#"+ fila).find('td[aria-describedby="' + ariaDescribedBy + '"]').text();
				...
				for (var i = 0; i<5; i++){
					let campoCantidad="p115_fld_cantidad"+i;
					let campoBulto="p115_fld_bulto"+i;
					let stock=data.listTDevolucionLinea[i].stock;
					if(stock!=null){
					...
					if (i==0 && stock == null && bulto != null){
						...
						document.getElementById(campoBulto).value=data.listTDevolucionLinea[i].bulto;
					}

//					let esEstadoCerrado="N";
					let esEstadoCerrado=data.listTDevolucionLinea[i].estadoCerrado;
//
...
//	        		}
                	...
                	$("#p115_estadoCerrado"+i).val(esEstadoCerrado);

					// si Bulto Cerrado hacemos no editables los campos "Cantidad a Devolver" y "Bulto".
					if (esEstadoCerrado=="S"){
						$("#p115_fld_cantidad"+i).css("background", "lightgray");
						$("#p115_fld_cantidad"+i).attr('readonly', true);
						$("#p115_fld_bulto"+i).css("background", "lightgray");
						$("#p115_fld_bulto"+i).attr('readonly', true);
						$("#p115_btn_eliminar"+i).hide();
					}else{
						$("#p115_fld_cantidad"+i).css("background", "white");
						$("#p115_fld_cantidad"+i).attr('readonly', false);
						$("#p115_fld_bulto"+i).css("background", "white");
						$("#p115_fld_bulto"+i).attr('readonly', false);
						$("#p115_btn_eliminar"+i).show();
					}
					
					$("#p115_fld_bulto"+i).change(function() {
						controlChangeBulto(this.id);
					});

					$("#p115_fld_bulto"+i).focus(function() {
...
function createParamsUrl(ref,devolucionPantalla){
...
function eliminarLinea(indice){
	var botonEliminarId="p115_btn_eliminar"+indice
	...
	if (eliminar != null && typeof(eliminar) != 'undefined') {
		for(var i = indice; i<4; i++){
			...
			var campoCantidad="p115_fld_cantidad"+i;
			var campoBulto="p115_fld_bulto"+i;
			var campoCantidadCopiar="p115_fld_cantidad"+idCopiar;
			var campoBultoCopiar="p115_fld_bulto"+idCopiar;
		...
		document.getElementById("p115_fld_cantidad4").value="";
		document.getElementById("p115_fld_bulto4").value="";
...
function validarCantidadesyBultosP115(){
	var camposValidos=true;
	var cantidad0 = document.getElementById("p115_fld_cantidad0").value;
	var bulto0 = document.getElementById("p115_fld_bulto0").value;
	var cantidad1 = document.getElementById("p115_fld_cantidad1").value;
	var bulto1 = document.getElementById("p115_fld_bulto1").value;
	var cantidad2 = document.getElementById("p115_fld_cantidad2").value;
	var bulto2 = document.getElementById("p115_fld_bulto2").value;
	var cantidad3 = document.getElementById("p115_fld_cantidad3").value;
	var bulto3 = document.getElementById("p115_fld_bulto3").value;
	var cantidad4 = document.getElementById("p115_fld_cantidad4").value;
	var bulto4 = document.getElementById("p115_fld_bulto4").value;
	...
	if((cantidad0=="" && bulto0=="") && (cantidad1=="" && bulto1=="") && (cantidad2=="" && bulto2=="") && (cantidad3=="" && bulto3=="") && (cantidad4=="" && bulto4=="")){
		...
		for(var i = 0; i<5 && camposValidos; i++){
			...
			camposValidos=validarCamposPantallaP115(cantidadPantalla,bultoPantalla);
		}
	}
	return camposValidos;
}

function validarCamposPantallaP115(cantidad,bulto){
	...
	//Validación para ver si los 2 registros están vacios
		...
		var flgPesoVariable=document.getElementById("p115_fld_flgPesoVariable").value;
...
function controlFocusBulto(idActual){
	...
	let filaBultoActual = "#p115_fld_bulto"+fila;
	let filaStockDevueltoActual = "#p115_fld_cantidad"+fila;
	...
	$("#p115_lbl_Error5").css("display", "none");
...
//		let valoresBultoAbierto=$("#"+fila+"_bultoAbierto").val();
		let valoresBultoAbierto=$("#p115_listaBultos").val();
		...
		if (ultimoBultoAbierto != -1
			...
			$("#p115_lbl_Error5").css("display", "block");
		}
	
	}
}

function events_115_validarBulto(indice){
	
	let campoCantidad="p115_fld_cantidad"+indice;
	let campoBulto="p115_fld_bulto"+indice;
...
//	let bultoCerrado=document.getElementById(campoBulto).className.indexOf('readonly');
	...
	if (!document.getElementById(campoCantidad).hasAttribute("readonly")){
		...
		if (validacionBultoP115(indice)=='S' && valorBulto!=''){
			document.getElementById("p115_lbl_Error").style.display = "none";
			let listaBultos=document.getElementById('p115_listaBultos').value;
			...
			if (!bultoValido){
				document.getElementById(campoBulto).value="";
				document.getElementById("p115_lbl_Error5").style.display = "block";
				document.getElementById("p115_lbl_Error").style.display = "none";
				document.getElementById("p115_lbl_Error2").style.display = "none";
				document.getElementById("p115_lbl_Error4").style.display = "none";
//				document.getElementById(campoBultoCaja).style.display = "none";
//				var listaBultos=document.getElementById('p115_listaBultos').value;
//				let aux = listaBultos.split(","); 
				for(var i = 0; i<aux.length && document.getElementById(campoBulto).value=="" ; i++){
					let existebulto=false;
					for(var j = 0; j<5 ; j++){
						let campoBultoComp="p115_fld_bulto"+j;
...
//				}
				...
				let valorEstadoCerrado="p115_estadoCerrado"+indice;
...
function events_115_calcularBulto(indice){

	//Obtención de fila y columna actuales
//	let indice = idActual.charAt(idActual.length - 1);

	let filaBultoActual = "#p115_fld_bulto"+indice;

	let campoCantidad = "p115_fld_cantidad"+indice;
	let valorCantidad = document.getElementById(campoCantidad).value;
//	var campoBultoCaja = "pda_p103_btn_cajaBulto"+indice;
//	let bultoCerrado = document.getElementById(campoCantidad).className.indexOf('readonly');
	
	if (!document.getElementById(campoCantidad).hasAttribute("readonly")){
		
		if (validacionCantidadP115(indice) == 'S'){
...
//				document.getElementById(campoBultoCaja).style.display = "block";
				document.getElementById(valorEstadoCerrado).value='N';
				document.getElementById("p115_lbl_Error5").style.display = "none";
...
//				document.getElementById(campoBultoCaja).style.display = "none";
				...
				var valorEstadoCerrado="p115_estadoCerrado"+indice;
//				document.getElementById(campoBultoCaja).style.display = "block";
				document.getElementById(valorEstadoCerrado).value='N';
				document.getElementById("p115_lbl_Error5").style.display = "none";
			}
...
function events_p115_eliminar(indice){
	let campoCantidadEliminar='p115_fld_cantidad'+indice;
	let campoBultoEliminar='p115_fld_bulto'+indice;
//	let campoEstadoEliminar='p115_estadoCerrado'+indice;
...
//	document.getElementById(campoEstadoEliminar).value='N';
	$("#p115_fld_cantidad"+indice).val('');
	$("#p115_fld_bulto"+indice).val('');
	$("#p115_estadoCerrado"+indice).val('N');
...
function validacionCantidadP115(indice){
	
	const regexEntero = /^-?\d+$/;

	let resultadoValidacion = 'S';
	let id = 'p115_fld_cantidad'+indice;
...
//		let flgPesoVariablePantalla=document.getElementById(idx+'_flgPesoVariable').value;
		let flgPesoVariablePantalla=document.getElementById("p115_fld_flgPesoVariable").value;
...
function validacionBultoP115(indice){
	var resultadoValidacion = 'S';
	var idBulto = 'p115_fld_bulto'+indice;
