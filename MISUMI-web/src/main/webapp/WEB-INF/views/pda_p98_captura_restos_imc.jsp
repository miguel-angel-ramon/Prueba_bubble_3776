...
<c:choose>
	...
	<c:when test="${data.tratamientoVegalsa eq true}">
		<div id="pda_p98_datosReferencia_bloque5">
			<fieldset id="pda_p98_imagenComercial">
				<%-- INICIO BLOQUE LEYENDA --%>
				<c:choose>
					<c:when test="${data.flgColorImplantacion eq 'VERDE'}">
						<legend id="pda_p98_imagenComercialLegendVerde"> 
							<span>
								<a id="pda_p98_imagenComercialSpanVerde" href="./pdaP17implantacionPopup.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&implantacion=${data.implantacion}&flgColorImplantacion=${data.flgColorImplantacion}&origenGISAE=${origenGISAE}">
					...
					<c:when test="${data.flgColorImplantacion eq 'ROJO'}">
						<legend id="pda_p98_imagenComercialLegendRojo">
							<span>
								<a id="pda_p98_imagenComercialSpanRojo" href="./pdaP17implantacionPopup.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&implantacion=${data.implantacion}&flgColorImplantacion=${data.flgColorImplantacion}&origenGISAE=${origenGISAE}">
									<spring:message code="pda_p12_datosReferencia.implantacionCajas" />
								</a>
							</span>
						</legend>
					...
					<c:otherwise>
						<legend id="pda_p98_imagenComercialLegend">
							<spring:message code="pda_p12_datosReferencia.implantacionCajas" />
						</legend>
					</c:otherwise>
				</c:choose>
				...
				<%-- INICIO BLOQUE DATOS --%>
				<div id="pda_p98_datosReferencia_bloque5_areaPlanograma">
				
					<%-- BLOQUE CAP LIN y FAC LIN --%>
					<div id="pda_p98_datosReferencia_bloque5_facCap">
						<%-- CAPACIDAD SIN Link --%>						
						<div id="pda_p98_datosReferencia_bloque5_capacidad">						
							<label id="pda_p98_lbl_capacidad" class="etiquetaCampoNegrita">
								<spring:message code="pda_p12_datosReferencia.capacidadLineal" />
							</label>
							<label id="pda_p98_lbl_capacidadVal" class="valorCampo">
								${imc.capacidad}
							</label>
						</div>								
	
						<%-- VALOR de FACING SIN Link --%>
						<div id="pda_p98_datosReferencia_bloque5_facingDerecha">
							...
							<c:choose>
								...
								<c:when test="${imc.facingModificable eq 1}">
									<c:choose> 
										...
										<c:when test="${empty guardadoImc}">
											<a href="./pdaP33Imc.do?codArt=${data.codArt}&procede=pdaP98CapturaRestos">
								...
								<%-- SIN Link --%>
								<c:otherwise>
									<label id="pda_p98_lbl_facing" class="etiquetaCampoNegrita">
										<spring:message code="pda_p12_datosReferencia.facingLineal"/>&nbsp;
									</label>
									<label id="pda_p98_lbl_facingVal" class="valorCampo">
										${imc.facing}
									</label>
								</c:otherwise>
							</c:choose>
						...
						<div id="pda_p98_datosReferencia_bloque5_multipDerecha">
					...
					<c:when test="${data.flgColorImplantacion eq 'VERDE'}">
						<c:choose>
							<c:when test="${imc.tipoReferencia eq esCajaExpositoraRef}">
									<legend id="pda_p98_imagenComercialLegend">
										<a id="pda_p98_imagenComercialSpanVerde"
										href="./pdaP17implantacionPopup.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&implantacion=${data.implantacion}&flgColorImplantacion=${data.flgColorImplantacion}&origenGISAE=${origenGISAE}">
					...
					<c:choose>
						...
						<c:when test="${imc.tipoReferencia == esCajaExpositoraRef}">
							...
							<c:if test="${imc.facingAlto > 0}">
								<div id ="pda_p98_datosReferencia_bloque5_ImplCajas">
							...
							<div id="pda_p98_datosReferencia_bloque5_multipImcDerecha">
								...
								<%-- VALOR IMC --%>
								<div id="pda_p98_datosReferencia_bloque5_imcDerecha">
							...
							<c:if test = "${(data.pedidoActivo eq 'S') && (data.capacidad1 ne null && data.capacidad1 ne 0 && data.capacidad1 ne '') || (data.facing1 ne null && data.facing1 ne 0 && data.facing1 ne '')}">
								<div id="pda_p98_datosReferencia_bloque5_cap1fac1">
									<div id="pda_p98_datosReferencia_bloque5_cap1">
										<label id="pda_p98_lbl_capacidad1" class="etiquetaCampoNegrita"><spring:message code="pda_p12_datosReferencia.capacidad1" /></label>
										<label id="pda_p98_lbl_capacidad1Val" class="valorCampo">${data.capacidad1}</label>
									</div>
									<div id="pda_p98_datosReferencia_bloque5_fac1">
										<label id="pda_p98_lbl_facing1" class="etiquetaCampoNegrita"><spring:message code="pda_p12_datosReferencia.facing1" /></label>
										<label id="pda_p98_lbl_facing1Val" class="valorCampo">${data.facing1}</label>
									</div>
								</div>
							</c:if>		
	...
	<c:when test="${imc.metodo == esSfmRef}">
		<div id="pda_p98_datosReferencia_bloque5">
			<fieldset id="pda_p98_imagenComercial">
				...
				<c:choose>
					...
					<c:when test="${data.flgColorImplantacion eq 'ROJO'}">
						<c:choose>
							...
							<c:otherwise>
								<legend id="pda_p98_imagenComercialLegend">
									<a id="pda_p98_imagenComercialSpanRojo"
										href="./pdaP17implantacionPopup.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&implantacion=${data.implantacion}&flgColorImplantacion=${data.flgColorImplantacion}&origenGISAE=${origenGISAE}">
										<spring:message code="pda_p12_datosReferencia.implantacionFFPP" />
									</a>
								</legend>
				...
				<%-- INICIO BLOQUE DATOS --%>
				<div id="pda_p98_datosReferencia_bloque5_areaSfm">
					<div id="pda_p98_datosReferencia_bloque5_sfm">
						...
						<c:choose>
							...
							<c:when test="${data.pedir eq 'S'}">
								<c:choose>
									...
									<c:when test="${empty guardadoSfm}">
										<label id="pda_p98_lbl_sfm" class="etiquetaCampoNegrita"><a
											href="./pdaP21Sfm.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&origen=1&origenGISAE=${origenGISAE}"><spring:message
													code="pda_p12_datosReferencia.sfm" /></a></label>
										<label id="pda_p98_lbl_sfmVal" class="valorCampo">${imc.sfm}</label>
									</c:when>
									<%-- El link sale en verde. Significa que se ha ido a SFM y se ha vuelto tras guardar. --%>
									<c:otherwise>
										<label id="pda_p98_lbl_sfm" class="etiquetaCampoNegrita"><a
											href="./pdaP21Sfm.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&origen=1&origenGISAE=${origenGISAE}"><span
												class="pda_p98_enlaceSfmOk"><spring:message
							...
							<%-- Sin link --%>
							<c:otherwise>
								<label id="pda_p98_lbl_sfm" class="etiquetaCampoNegrita"><spring:message
			...
			<fieldset id="pda_p98_imagenComercial">
				...
				<div id="pda_p12_datosReferencia_bloque5_areaFacCap">
					...
					<div id="pda_p98_datosReferencia_bloque5_facCap">
						...
						<div id="pda_p98_datosReferencia_bloque5_capacidad">
							<label id="pda_p98_lbl_capacidad" class="etiquetaCampoNegrita"><spring:message
									code="pda_p12_datosReferencia.capacidadLineal" /></label> 
							<label id="pda_p98_lbl_capacidadVal" class="valorCampo">${imc.capacidad}</label>
						</div>
						<%-- VALOR DE FACING --%>
						<div id="pda_p98_datosReferencia_bloque5_facingDerecha">
							<c:choose>
								<%-- FACING CON LINK --%>
								<c:when test="${pageData.mmc eq 'S'}">
									<c:if test="${empty guardadoSfm }">
										<label id="pda_p98_lbl_facingEnlace"
											class="etiquetaCampoNegrita"><a
											href="./pdaP21Sfm.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&origen=1&origenGISAE=${origenGISAE}">
											<spring:message	code="pda_p12_datosReferencia.facingLineal" /></a></label>
										<label id="pda_p98_lbl_facingVal" class="valorCampo">${imc.facing}</label>
									</c:if>
								...
								<%-- FACING SIN LINK --%>
								<c:otherwise>
									<label id="pda_p98_lbl_facing" class="etiquetaCampoNegrita"><spring:message
	...
	<c:when test="${imc.metodo == esCapRef}">
		<div id="pda_p98_datosReferencia_bloque5">
			<fieldset id="pda_p98_imagenComercial">
				...
				<%-- INICIO BLOQUE DATOS --%>
				<div id="pda_p98_datosReferencia_bloque5_areaCap">
					<%-- VALOR DE CAPACIDAD Y VALOR DE FACING CON Y SIN LINK --%>
					<div id="pda_p98_datosReferencia_bloque5_facCap">
						...
						<div id="pda_p98_datosReferencia_bloque5_capacidad">
							<c:choose>
								...
								<c:when test="${pageData.mmc eq 'S'}">
									<c:if test="${empty guardadoSfm}">
										<label id="pda_p98_lbl_capacidadEnlace"
											class="etiquetaCampoNegrita"> <a
											href="./pdaP21Sfm.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&origen=1&origenGISAE=${origenGISAE}"><spring:message
									...
									<c:if test="${not empty guardadoSfm}">
										<label id="pda_p98_lbl_capacidadEnlace"
											class="etiquetaCampoNegrita"> <a
											href="./pdaP21Sfm.do?procede=pdaP98CapturaRestos&codArt=${data.codArt}&origen=1&origenGISAE=${origenGISAE}">
												<span class="pda_p98_enlaceSfmOk"><spring:message
	...
	<c:when test="${imc.metodo == esFacNoAliRef}">
		<div id="pda_p98_datosReferencia_bloque5">
			<fieldset id="pda_p98_imagenComercial">
				...
				<%-- FIN BLOQUE LEYENDA --%>
				<%-- INICIO CONTENIDO --%>
				<div id="pda_p98_datosReferencia_bloque5_areaFacing">
					<%-- VALOR FACING CON Y SIN LINK --%>
					<div id="pda_p98_datosReferencia_bloque5_facing">
						<c:choose>
							<%-- FACING CON LINK --%>
							<c:when test="${pageData.mmc eq 'S'}">
								<c:if test="${empty guardadoSfm }">
					...
					<%-- VALOR MULTIP E IMC --%>
					<div id="pda_p98_datosReferencia_bloque5_multipImc">
						<%-- VALOR MULTIP SI NO ES FFPP--%>
						<c:if test="${imc.tipoReferencia ne esFFPPRef}">
							<div id="pda_p98_datosReferencia_bloque5_multip">
								<label class="etiquetaCampoNegrita"> <spring:message
										code="pda_p12_datosReferencia.facingLinealVPlanoPMultipli" />
								</label> 
						...
						<%-- VALOR IMC --%>
						<div id="pda_p98_datosReferencia_bloque5_imc">
							<label class="etiquetaCampoNegrita"> <spring:message
									code="pda_p12_datosReferencia.legendIMCImplantacion1" />
							</label> 
