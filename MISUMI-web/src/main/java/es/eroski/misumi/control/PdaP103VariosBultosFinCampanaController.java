...
@Controller
public class PdaP103VariosBultosFinCampanaController extends pdaConsultasController{

	private static Logger logger = Logger.getLogger(PdaP103VariosBultosFinCampanaController.class);
	...
	@RequestMapping(value = "/pdaP103VariosBultosFinCampana",method = RequestMethod.GET)
	public String showForm(ModelMap model
						  , @Valid final Long codArt, @Valid final Long devolucionPantalla
						  , @Valid final Double stockDevueltoPantalla, @Valid @Nullable final Long bultoPantalla
						...
						if(devolucionCatalogoEstadoActualizada.getListDevolucionEstado() != null && devolucionCatalogoEstadoActualizada.getListDevolucionEstado().size() > 0){
							if(devolucionCatalogoEstadoActualizada.getListDevolucionEstado().get(0).getListDevolucion() != null && devolucionCatalogoEstadoActualizada.getListDevolucionEstado().get(0).getListDevolucion().size() > 0){
								...
								if(new Long("0").equals(devolucionActualizada.getCodError()) || devolucionActualizada.getCodError() == null){
									...
									String err = "ERR_ACT";
									model.addAttribute("actionP67Volver","pdaP62VariosBultosLinkVuelta.do");
		...
		listLineasTotal = this.devolucionesService.findLineasDevolucion(session.getId(),devolucion,null,null,null,null);						
		listLineasTotal = this.devolucionLineaBultoCantidadService.cargarBultoCantidadLinea(session.getId(),listLineasTotal);
		String listaBultos=this.devolucionLineaBultoCantidadService.cargarListaBultos(session.getId(),list.get(0));
		Double sumaEuros = getCalcularSumaEuros(listLineasTotal);
		...
		devolucionLinea.setCosteUnitario(list.get(0).getCosteUnitario());
	...
	@RequestMapping(value = "/pdaP103VariosBultosFinCampana",method = RequestMethod.POST)
	public String processForm(@Valid final PdaDatosCabecera pdaDatosCab, ModelMap model, RedirectAttributes redirectAttributes,
		...
		String resultado = "redirect:pdaP62VariosBultosLinkVuelta.do";
		redirectAttributes.addAttribute("codArt", devolucionLinea.getCodArticulo());
		String accion = request.getParameter("action");

		String mostrarFinDev = request.getParameter("mostrarFinDev");
		...
		if (accion!=null && accion.equals("save")){
			try {
				...
				this.insertarDatosTablaSesionLineaBulto(devolucionLinea,session.getId());
				...
				List<TDevolucionLinea> listTDevolucionLinea = this.devolucionesService.findLineasDevolucion(session.getId(),devolucion,null,null,null,null);						
				...
				if (devolucion.getCosteMaximo()!=null && devolucion.getCosteMaximo() > 0L){
					...
					if (sumaEuros.compareTo(cotaMaxima) > 0){
						...
						errorAlValidar = true;
						model.addAttribute("codErrorActualizar",err);
						session.setAttribute("codArticuloDevolucion", devolucionLinea.getCodArticulo());
		...
		redirectAttributes.addAttribute("mostrarFinDev", mostrarFinDev);
