...
@Controller
@RequestMapping("/misPedidos/cantidadesConfirmadas")
public class p23MisPedidosCantConfController {
	
	private static Logger logger = Logger.getLogger(p23MisPedidosCantConfController.class);
	...
	@Autowired
	private VConfirmacionEnviadoService vConfirmacionEnviadoService;
	@Autowired
	...
	public String showForm(Map<String, String> model) {
		...
		return "p21_popupCantidadesConfirmadas";
	}
	...
	public  @ResponseBody Page<SeguimientoMiPedidoDetalle> loadDataGrid(
			...
			try {											
				list = this.vConfirmacionEnviadoService.findSeguimientoMiPedido(seguimientoMiPedido, pagination);
			} catch (Exception e) {
			...
			if (list != null) {
				int records = this.vConfirmacionEnviadoService.findSeguimientoMiPedidoCont(seguimientoMiPedido).intValue();
	...
	void exportGrid(
		...
		try {
		    ...
		    seguimientoMiPedidoDetalle.setConfTotalRefBajoPedido(this.vConfirmacionEnviadoService.findTotalReferenciasBajoPedido(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalCajasBajoPedido(this.vConfirmacionEnviadoService.findTotalCajasBajoPedido(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalRefEmpuje(this.vConfirmacionEnviadoService.findTotalReferenciasEmpuje(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalCajasEmpuje(this.vConfirmacionEnviadoService.findTotalCajasEmpuje(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalRefImplCab(this.vConfirmacionEnviadoService.findTotalReferenciasImplCab(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalCajasImplCab(this.vConfirmacionEnviadoService.findTotalCajasImplCab(seguimientoMiPedido));
		    
		    seguimientoMiPedidoDetalle.setConfTotalRefIntertienda(this.vConfirmacionEnviadoService.findTotalReferenciasIntertienda(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setConfTotalCajasIntertienda(this.vConfirmacionEnviadoService.findTotalCajasIntertienda(seguimientoMiPedido));
		    Boolean isCentroVegalsa = this.vConfirmacionEnviadoService.isCentroVegalsa(user.getCentro().getCodCentro());
		    
			try {
				logger.info("Exporting excel Pedidos cantidades confirmadas:"+model.toString());
								
				list = this.vConfirmacionEnviadoService.findSeguimientoMiPedidoExcel(seguimientoMiPedido, model);
			...
			this.excelManager.exportMisPedidosCantConf(list, headers, model, this.messageSource, user.getCentro(), fechaPedidoDDMMYYYY, vAgruComerGrupoArea, vAgruComerGrupoSection, vAgruComerGrupoCategory, seguimientoMiPedidoDetalle, isCentroVegalsa, response);
