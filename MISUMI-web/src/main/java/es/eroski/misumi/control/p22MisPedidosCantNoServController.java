...
@Controller
@RequestMapping("/misPedidos/cantidadesNoServidas")
public class p22MisPedidosCantNoServController {
	
	private static Logger logger = Logger.getLogger(p22MisPedidosCantNoServController.class);
	...
	public String showForm(Map<String, String> model) {
		...
		return "p22_popupCantidadesNoServidas";
	}
	...
	public  @ResponseBody Page<SeguimientoMiPedidoDetalle> loadDataGrid(
			...
			try {   							
				list = this.vConfirmacionNoServidoService.findSeguimientoMiPedido(seguimientoMiPedido, pagination);
			} catch (Exception e) {
			...
			if (list != null) {
				int records = this.vConfirmacionNoServidoService.findSeguimientoMiPedidoCont(seguimientoMiPedido).intValue();
	...
	void exportGrid(
		...
		try {
		    ...
		    seguimientoMiPedidoDetalle.setNsrTotalRefNoServidas(this.vConfirmacionNoServidoService.findTotalReferenciasNoServidas(seguimientoMiPedido));
		    seguimientoMiPedidoDetalle.setNsrTotalCajasNoServidas(this.vConfirmacionNoServidoService.findTotalCajasNoServidas(seguimientoMiPedido));
		    Boolean isCentroVegalsa = this.vConfirmacionNoServidoService.isCentroVegalsa(user.getCentro().getCodCentro());
		    
			try {
				logger.info("Exporting excel Pedidos cantidades no servidas:"+model.toString());				
				
				list = this.vConfirmacionNoServidoService.findSeguimientoMiPedidoExcel(seguimientoMiPedido, model);				
			} catch (Exception e) {
				//logger.error(StackTraceManager.getStackTrace(e));
				response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
				throw e;
			...
			this.excelManager.exportMisPedidosCantNoServ(list, headers, model, this.messageSource, user.getCentro(), fechaPedidoDDMMYYYY, vAgruComerGrupoArea, vAgruComerGrupoSection, vAgruComerGrupoCategory, seguimientoMiPedidoDetalle, isCentroVegalsa, response);
